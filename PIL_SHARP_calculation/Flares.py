...
def main():
    ...
    for i in range(len(SHARP_original)):
        ...
        correlation_lst.append([sharp_name[i], So_series.corr(flare_series), Sm_series.corr(flare_series), Soc_series.corr(flare_series)])

    with open(r'res/SHARP_csv/csv_correlation/correlation1.csv', 'w', newline='') as csvfile:
        spamwriter = csv.writer(csvfile)
        spamwriter.writerow(['SHARP', 'original', 'pil-masked'])
    ...
    for i in range(len(SHARP_masked)):
        plot_together(SHARP_masked[i], flare_env_masked, Time_SHARP_masked, sharp_name[i], 'pilmasked')
