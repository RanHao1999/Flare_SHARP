"""
This code is for calculating SHARP keys with the pilmask put on.
"""
...
def compute_abs_flux(bz, bz_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
    ...
    pilmask is the list of the coordinates of the pil-mask.
    """
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            if np.isnan(bz[j, i]):
                continue
            if [j,i] in pilmask: # points in pil-mask area
                sum += abs(bz[j, i])
...
def compute_bh(bx, by, bz, bx_err, by_err, bz_err, conf_disambig, bitmap, nx, ny, pilmask):
    """function: compute_bh
...
def compute_gamma(bx, by, bz, bh, bz_err, bh_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
...
def compute_bt(bx, by, bz, bx_err, by_err, bz_err, conf_disambig, bitmap, nx, ny, pilmask):
    """function: compute_bt
...
def computeBtderivative(bt, bt_err, nx, ny, conf_disambig, bitmap, pilmask):
...
def computeBhderivative(bh, bh_err, nx, ny, conf_disambig, bitmap, pilmask):
...
def computeBzderivative(bz, bz_err, nx, ny, conf_disambig, bitmap, pilmask):
...
def computeJz(bx, by, bx_err, by_err, conf_disambig, bitmap, nx, ny, pilmask):
    """function: computeJz
...
def computeJzmoments(jz, jz_err, derx, dery, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, munaught, pilmask):
...
def computeAlpha(jz, jz_err, bz, bz_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
...
def computeHelicity(jz, jz_err, bz, bz_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
...
def computeSumAbsPerPolarity(jz, jz_err, bz, bz_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, munaught, pilmask):
...
def computeFreeEnergy(bx_err, by_err, bx, by, bpx, bpy, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, conf_disambig, bitmap, pilmask):
    """
...
def computeShearAngle(bx_err, by_err, bz_err, bx, by, bz, bpx, bpy, nx, ny, conf_disambig, bitmap, pilmask):
    """
...
def computeR(los, los_err, nx, ny, cdelt1_arcsec, pilmask):
    """
    function: computeR
...
def computeLOSderivative(los, los_err, nx, ny, bitmap, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
...
def compute_abs_flux_los(los, los_err, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, pilmask):
