...
import os, csv
import pandas as pd
import sunpy, sunpy.map, scipy, sys, math, argparse, normal_funcs as nf
import concurrent.futures
import numpy as np
from Calculate_sharpkeys import compute_abs_flux, compute_bh, compute_gamma,\
...
def get_data(input):
    ...
    """
    [file_bz, file_by, file_bx, file_bz_err, file_by_err, file_bx_err, file_conf_disambig, file_bitmap, file_los, count] = input
    ...
    return [bz, by_flipped, bx, bz_err, by_err, bx_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs,
            cdelt1_arcsec, los, los_err, date, count]

def main_run(input):
    [bz, by, bx, bz_err, by_err, bx_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, los, los_err, date, count] = get_data(input)
    print("Following results are calculated manully from .fits files")
    ...
    with open('res/SHARP_csv/SHARP_original_calculated_' + str('%04d' % count) + '.csv', 'w', newline='') as csvfile:
