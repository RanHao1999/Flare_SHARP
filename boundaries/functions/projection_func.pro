pro projection_func,date1,time1

time_lst = [date1,'_',time1]
filename1 = 'F:\3D-magnetic-con\3d_myrun\02ambiguity_hmi\01plot_hmi\'+strjoin(time_lst)
filename2 = 'F:\3D-magnetic-con\3d_myrun\02ambiguity_hmi\05projection\'+strjoin(time_lst)
spawn,'mkdir '+filename2

restore,filename1+'\'+'bxyz_submap.sav',/ver
mapbz=smapbz
mapbx=smapbx
mapby=smapby
...
files = find_file(filename2+'\'+'cmd_lat.sav',count = count)
if (count eq 0) then begin
  ...
  save,cmd1,latitude,filename=filename2+'\'+'cmd_lat.sav'
endif else begin
  restore,filename2+'\'+'cmd_lat.sav',/ver
endelse
...
save,mapbx,mapby,mapbz,filename=filename2+'\'+'mapbxyz_m.sav'
wdef,1,1300,800
!p.background=255
mapbz.id='Heliographic Bxyz'
plot_map,mapbz,color=0,bcolor=0,charsize=2.0,dmax=800,dmin=-800
write_png,filename2+'\'+'bz_m.png',tvrd()
...
!p.background=0
write_png,filename2+'\'+'bxyz_m.png',tvrd()
